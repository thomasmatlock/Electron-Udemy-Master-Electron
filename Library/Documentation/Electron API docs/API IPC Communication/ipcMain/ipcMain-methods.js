...
ipcMain.handle(channel, listener); //Adds a handler for an invocable IPC.This handler will be called whenever a renderer calls ipcRenderer.invoke(channel, ...args).
ipcMain.handleOnce(channel, listener); //Handles a single invocable IPC message, then removes the listener.See ipcMain.handle(channel, listener).
